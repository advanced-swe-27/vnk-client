...
import { DELETE_FACILITY } from "@/utils/server/facility"

type Props = {
    facility: FacilityRes
}

export default function DeleteFacilityDialog({ facility }: Props) {
    const { _id, createdAt, name, updatedAt, description } = facility
    ...
    const approveResident = useMutation({
        mutationFn: () => {
            if (user && user.token) {
                return DELETE_FACILITY(_id, user.token)
            }
        ...
        onError: (error: any) => {
            toast.error(error?.response?.data?.message || "Couldn't delete facility. Try again later")
        }
    })
            ...
            <Dialog>
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Are you sure you want to delete room <span className="font-medium text-neutral-500"> {name}</span> This action can not be undone and all data pertaining to this room will be permanently removed from our servers
                        </DialogDescription>
                    </DialogHeader>
